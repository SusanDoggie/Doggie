...
public struct TriangularPatchTessellator2 : ImageContextRenderTriangleGenerator {
    ...
    public func render(projection: (Vector) -> Point, _ body: (Vector, Vector, Vector) -> Void) {
        ...
        TriangularPatchTessellator2(patch: patch, q0, s0, s2)._render(projection: projection, body)
        TriangularPatchTessellator2(patch: patch, s0, q1, s1)._render(projection: projection, body)
        TriangularPatchTessellator2(patch: patch, s2, s1, q2)._render(projection: projection, body)
        TriangularPatchTessellator2(patch: patch, s0, s1, s2)._render(projection: projection, body)
    }
}
...
public func tessellation2(width: Int, height: Int) -> Image<ARGB32ColorPixel> {
    ...
    context.render(list.map { TriangularPatchTessellator2(patch: $0 * matrix) }, projection: PerspectiveProjectMatrix(angle: 5 * .pi / 18, nearZ: 1, farZ: 500), shader: shader)
