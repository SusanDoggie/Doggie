...
docker run --env SWIFT_SNAPSHOT -v ${TRAVIS_BUILD_DIR}:${TRAVIS_BUILD_DIR} ${DOCKER_IMAGE} /bin/bash -c "apt-get update && apt-get install -y sudo git wget && cd $TRAVIS_BUILD_DIR && ./.travis-build.sh"
exit $?
